...
class ViewController: UITableViewController {
    ...
    private func setupNavigationBar(){
        ...
        if #available(iOS 13.0, *){
            let titleImage = UIImage(systemName: "info")
...
//            if let indexPath = tableView.indexPathsForRow(at: touchPoint){
//                basicActionSheet(title: countries[indexPath.row].name, mesage: countries[indexPath.row].capital)
//            }
//
//
//        }
...
extension ViewController{
    ...
    private func basicActionSheet(title: String?, mesage: String?){
        DispatchQueue.main.async {
            let actionSheet: UIAlertController = UIAlertController(title: title, message: mesage, preferredStyle: .actionSheet)
